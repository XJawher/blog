...
为了进一步防止 CA 证书被篡改引发安全问题，引入了数字签名。
数字签名就是使用 CA 自带的 HASH 算法对证书的内容做 hash 得到一个摘要，再用 CA 的私钥加密最终组成数字签名。当别人把他的 CA 证书发过来的时候，再用同样的 HASH 算法，再次生成消息摘要，然后用 CA 的公钥对消息签名做解密，得到 CA 创建的消息摘要，这俩做个对比就得到了 CA 的证书有没有被篡改的情况。
